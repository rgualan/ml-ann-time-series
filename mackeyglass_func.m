function [ T, X ] = mackeyglass_func(sample_n, a_param, b_param, tau_param, x0_param, deltat_param)
    %% mackeyglass
    ...
    if exist('a_param', 'var'); a = a_param; end;
    
    b        = 0.1;     % value for b in eq (1)
    if exist('b_param', 'var'); b = b_param; end;

    tau      = 17;		% delay constant in eq (1) 17
    if exist('tau_param', 'var'); tau = tau_param; end;
    
    x0       = 1.2;		% initial condition: x(t=0)=x0 1.2
    if exist('x0_param', 'var'); x0 = x0_param; end;

    deltat   = 0.1;	    % time step size (which coincides with the integration step) 0.1
    if exist('deltat_param', 'var'); deltat = deltat_param; end;
