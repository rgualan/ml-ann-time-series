...
figure(1),clf;
plot(TC, '-', 'LineWidth', 2); hold on; 
plot(size(TX1,1)-size(yhtr,1)+1:size(TX1,1), yhtr, 'LineWidth', 2); 
...
%RMSE = sqrt(mean((TC(end) - yhts).^2)); %RMSE makes no sense here. Only one item
ERROR=yhts-TC(end);
display(ERROR);